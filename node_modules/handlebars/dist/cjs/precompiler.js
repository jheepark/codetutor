...
let _async2 = _interopRequireDefault(_async);
...
let Handlebars = _interopRequireWildcard(_handlebars);

let _path = require('path');

let _sourceMap = require('source-map');

let _uglifyJs = require('uglify-js');

let _uglifyJs2 = _interopRequireDefault(_uglifyJs);
...
function loadFiles(opts, callback) {
  ...
  _async2['default'].whilst(function () {
    ...
    let _queue$shift = queue.shift();

    let path = _queue$shift.template;
    let root = _queue$shift.root;
...
module.exports.cli = function (opts) {
  ...
  let multiple = opts.templates.length !== 1 || opts.hasDirectory;
  ...
  let objectName = opts.partial ? 'Handlebars.partials' : 'templates';

  let output = new _sourceMap.SourceNode();
  if (!opts.simple) {
    if (opts.amd) {
      output.add('define([\'' + opts.handlebarPath + 'handlebars.runtime\'], function(Handlebars) {\n  Handlebars = Handlebars["default"];');
    } else if (opts.commonjs) {
      output.add('let Handlebars = require("' + opts.commonjs + '");');
    } else {
      output.add('(function() {\n');
    }
    output.add('  let template = Handlebars.template, templates = ');
    if (opts.namespace) {
  ...
  opts.templates.forEach(function (template) {
    ...
    if (opts.map) {
      let consumer = new _sourceMap.SourceMapConsumer(precompiled.map);
