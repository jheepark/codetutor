'use strict';

exports.__esModule = true;
// istanbul ignore next

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { 'default': obj }; }

let _handlebarsRuntime = require('./handlebars.runtime');

let _handlebarsRuntime2 = _interopRequireDefault(_handlebarsRuntime);

// Compiler imports

let _handlebarsCompilerAst = require('./handlebars/compiler/ast');

let _handlebarsCompilerAst2 = _interopRequireDefault(_handlebarsCompilerAst);

let _handlebarsCompilerBase = require('./handlebars/compiler/base');

let _handlebarsCompilerCompiler = require('./handlebars/compiler/compiler');

let _handlebarsCompilerJavascriptCompiler = require('./handlebars/compiler/javascript-compiler');

let _handlebarsCompilerJavascriptCompiler2 = _interopRequireDefault(_handlebarsCompilerJavascriptCompiler);

let _handlebarsCompilerVisitor = require('./handlebars/compiler/visitor');

let _handlebarsCompilerVisitor2 = _interopRequireDefault(_handlebarsCompilerVisitor);

let _handlebarsNoConflict = require('./handlebars/no-conflict');

let _handlebarsNoConflict2 = _interopRequireDefault(_handlebarsNoConflict);

let _create = _handlebarsRuntime2['default'].create;
function create() {
  let hb = _create();

  hb.compile = function (input, options) {
    return _handlebarsCompilerCompiler.compile(input, options, hb);
  };
  hb.precompile = function (input, options) {
    return _handlebarsCompilerCompiler.precompile(input, options, hb);
  };

  hb.AST = _handlebarsCompilerAst2['default'];
  hb.Compiler = _handlebarsCompilerCompiler.Compiler;
  hb.JavaScriptCompiler = _handlebarsCompilerJavascriptCompiler2['default'];
  hb.Parser = _handlebarsCompilerBase.parser;
  hb.parse = _handlebarsCompilerBase.parse;

  return hb;
}

let inst = create();
inst.create = create;

_handlebarsNoConflict2['default'](inst);

inst.Visitor = _handlebarsCompilerVisitor2['default'];

inst['default'] = inst;

exports['default'] = inst;
module.exports = exports['default'];
//# sourceMappingURL=data:application/json;base64,
