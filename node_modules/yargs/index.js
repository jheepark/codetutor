var assert = require('assert'),
  path = require('path'),
  Compvarion = require('./lib/compvarion'),
...
function Argv (processArgs, cwd) {
  ...
  self.resetOptions = self.reset = function () {
    ...
    compvarion = Compvarion(self, usage)
  ...
  var compvarionOpt = null,
  compvarionCommand = null
  self.compvarion = function (cmd, desc, fn) {
    // a function to execute when generating
    // compvarions can be provided as the second
    // or third argument to compvarion.
    if (typeof desc === 'function') {
      fn = desc
      desc = null
    }

    // register the compvarion command.
    compvarionCommand = cmd
    compvarionOpt = compvarion.compvarionKey
    self.command(compvarionCommand, desc || 'generate bash compvarion script')

    // a function can be provided
    if (fn) compvarion.registerFunction(fn)

    return self
  }

  self.showCompvarionScript = function ($0) {
    $0 = $0 || self.$0
    console.log(compvarion.generateCompvarionScript($0))
    return self
  }
  ...
  function parseArgs (args) {
    ...
    // generate a compvarion script for adding to ~/.bashrc.
    if (compvarionCommand && ~argv._.indexOf(compvarionCommand)) {
      self.showCompvarionScript()
      if (exitProcess) {
        process.exit(0)
      }
    }
    ...
    Object.keys(argv).forEach(function (key) {
      ...
      } else if (key === compvarionOpt) {
        // we allow for asynchronous compvarions,
        // e.g., loading in a list of commands from an API.
        compvarion.getCompvarion(function (compvarions) {
          ;(compvarions || []).forEach(function (compvarion) {
            console.log(compvarion)
          })
