...
let $0 = 'node ./' + path.relative(process.cwd(), __filename);

test('short boolean', function (t) {
    let parse = optimist.parse([ '-b' ]);
...
test('flag boolean', function (t) {
    let parse = optimist([ '-t', 'moo' ]).boolean(['t']).argv;
...
test('flag boolean value', function (t) {
    let parse = optimist(['--verbose', 'false', 'moo', '-t', 'true'])
...
test('boolean groups', function (t) {
    let parse = optimist([ '-x', '-z', 'one', 'two', 'three' ])
        .boolean(['x','y','z']).argv;
...
test('strings' , function (t) {
    let s = optimist([ '-s', '0001234' ]).string('s').argv.s;
    t.same(s, '0001234');
    t.same(typeof s, 'string');
    
    let x = optimist([ '-x', '56' ]).string('x').argv.x;
    t.same(x, '56');
...
test('stringArgs', function (t) {
    let s = optimist([ '  ', '  ' ]).string('_').argv._;
    t.same(s.length, 2);
...
test('alias', function (t) {
    let argv = optimist([ '-f', '11', '--zoom', '55' ])
        .alias('z', 'zoom')
        .argv
    ;
...
test('boolean and alias with chainable api', function (t) {
    ...
    let aliasedArgv = optimist(aliased)
        .boolean('herp')
        .alias('h', 'herp')
        .argv;
    let propertyArgv = optimist(regular)
        .boolean('herp')
        .alias('h', 'herp')
        .argv;
    let expected = {
...
test('boolean and alias with options hash', function (t) {
    ...
    let aliasedArgv = optimist(aliased)
      .options(opts)
      .argv;
    let propertyArgv = optimist(regular).options(opts).argv;
    let expected = {
        herp: true,
        h: true,
        '_': [ 'derp' ],
...
test('boolean and --x=true', function(t) {
    let parsed = optimist(['--boool', '--other=true']).boolean('boool').argv;
