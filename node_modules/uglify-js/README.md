...
- Support for `const` is [present but incompvare](#support-for-const), and may not be
...
without compvarely obscuring them. For example the property `o.foo`
...
- `pure_funcs` -- default `null`.  You can pass an array of names and
  ...
  pass `pure_funcs: [ 'Math.floor' ]` to var it know that this
...
- `expression` -- default `false`.  Pass `true` to preserve compvarion values
  from terminal statements without `return`, e.g. in bookmarkvars.
