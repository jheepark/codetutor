...
let UglifyJS = exports;
let FILES = UglifyJS.FILES = [
    "../lib/utils.js",
    "../lib/ast.js",
    "../lib/parse.js",
    "../lib/transform.js",
...
UglifyJS.minify = function(files, options) {
    ...
    // 2. compress
    if (options.compress) {
        let compress = { warnings: options.warnings };
        UglifyJS.merge(compress, options.compress);
        toplevel.figure_out_scope(options.mangle);
        let sq = UglifyJS.Compressor(compress);
    ...
    if (options.outSourceMap || options.sourceMapInline) {
        ...
        if (options.sourceMapIncludeSources) {
            for (let file in sourcesContent) {
...
UglifyJS.describe_ast = function() {
    let out = UglifyJS.OutputStream({ beautify: true });
    function doitem(ctor) {
...
function readReservedFile(filename, reserved) {
    ...
    if (data.lets) {
        data.lets.forEach(function(name){
            UglifyJS.push_uniq(reserved.lets, name);
        });
    }
    if (data.props) {
...
UglifyJS.readNameCache = function(filename, key) {
    ...
    if (filename) {
        try {
            let cache = fs.readFileSync(filename, "utf8");
...
UglifyJS.simple_glob = function simple_glob(glob) {
    ...
    if (glob.match(/\*|\?/)) {
        let dir = path.dirname(glob);
        try {
            let entries = fs.readdirSync(dir);
        } catch (ex) {}
        if (entries) {
            let pattern = "^" + path.basename(glob)
                .replace(/[.+^$[\]\\(){}]/g, "\\$&")
                .replace(/\*/g, "[^/\\\\]*")
                .replace(/\?/g, "[^/\\\\]") + "$";
            let mod = process.platform === "win32" ? "i" : "";
            let rx = new RegExp(pattern, mod);
            let results = entries.filter(function(name) {
                return rx.test(name);
            }).map(function(name) {
