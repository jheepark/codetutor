...
function middleware(filename, options, cb) {
  ...
  let layout_filename = [].concat(view_dirs).map(function (view_dir) {
  ...
  let layout_template = layout_filename.reduce(function (cached, filename) {
    if (cached) {
      return cached;
    }

    let cached_file = cache[filename];
...
Instance.prototype.compile = function (str) {
  ...
  let template = this.handlebars.compile(str);
...
Instance.prototype.registerPartials = function (directory, done) {
  ...
  let register = function(filepath, done) {
    let isValidTemplate = /\.(html|hbs)$/.test(filepath);
